clc,clear,close all;
warning off
global popsize lengthN min max N code;

GS=350;%射线条数
H=20;%行数目
L=50;%列数目
N=H*L;%每个个体包含100个数待优化 % 每个染色体段数（十进制编码位数）
% N=12;                % 每个染色体段数（十进制编码位数）
M=50;               % 进化代数
popsize=20;          % 设置初始参数，群体大小
lengthN=20;           % length为每段基因的二进制编码位数
chromlength=N*lengthN;  % 字符串长度（个体长度），染色体的二进制编码长度
pc=0.7;                % 设置交叉概率，本例中交叉概率是定值，若想设置变化的交叉概率可用表达式表示，或从写一个交叉概率函数，例如用神经网络训练得到的值作为交叉概率
pm=0.3;                % 设置变异概率，同理也可设置为变化的
% bound={0.02*ones(popsize,1),0.13*ones(popsize,1)};
...
while k<M
      ...
      datetime
       ...
       if bestfit<ymax
          ymax=bestfit;
           for j=1:N  %译码！
               temp(:,j)=decodechrom(bestindividual,1+(j-1)*lengthN,lengthN);      %将newpop每行(个体）每列（每段基因）转化成十进制数
               x(:,j)=temp(:,j)/(2^lengthN-1)*(max-min)+min;     % popsize×N 将二值域中的数转化为变量域的数       
           end
          ysw_x(i,:) = x;  %译码！
       end
       y(i,k)=ymax;
%        if ymax<0.1     % 如果最大值小于设定阀值，停止进化
%            break
%        end
    k=k+1;
       pop=newpop;
